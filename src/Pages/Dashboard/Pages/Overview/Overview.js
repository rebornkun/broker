            ...
            <div className='fdcolumn gap02 jcsevenly'>
                ...
                <div className='fdrow gap1'>
                    <div className='fdrow aic gap02'>
                        ...
                        <p className='value'>{userData[0]?.current_plan ? userData[0]?.current_plan : 'none'}</p>
                    </div>
                    <div className='fdrow aic gap02'>
                        <p className='montitle'>USD:</p>
                        <p className='value'>${userData[0]?.USD}.0</p>
                    </div>
                    <div className='fdrow aic gap02'>
       ...
       <div className='profit box_div fdrow'>
            <div className='fdrow aic gap02'>
                ...
                { Number.isInteger(userData[0]?.earned) ?
                    <p className='value'>${userData[0]?.earned}.0</p>
                    :
                    <p className='value'>${userData[0]?.earned?.toFixed(1)}</p>
                }
            </div>
            <div className='fdrow aic gap02'>
                <p className='montitle'>Available:</p>
                { Number.isInteger(userData[0]?.available) ?
                    <p className='value'>${userData[0]?.available}.0</p>
                    :
            ...
            <div className='fdrow aic gap02'>
                ...
                { Number.isInteger(userData[0]?.paid) ?
                    <p className='value'>${userData[0]?.paid}.0</p>
                    :
                    <p className='value'>${userData[0]?.paid?.toFixed(1)}</p>
                }
            </div>
       </div> 
    );
...
const PlanBox = ({userData}) => {
    ...
    const updateProfit = () => {
        ...
        let lastProfitTime = new Date(userData[0]?.last_profit_added_time?.toDate()).getTime();
    ...
    const updateProfitValue = async(amount) => {
        ...
        try{
            ...
            await updateDoc(userDoc, { plan_profit: userData[0]?.plan_profit + amount, last_profit_added_time: date})
    ...
    const calculate = async(amount) => {
        if ( NOW_IN_MS - THEN_IN_MS > ONE_DAY_IN_MS){
            ...
            try{
                const userDoc = doc(db, "User", userData[0]?.id)
                await updateDoc(userDoc, { current_plan: '', plan_profit: 0, plan_buy_time: '', last_profit_added_time: '', available: userData[0]?.available + userData[0]?.plan_profit + amount, earned: userData[0]?.earned + userData[0]?.plan_profit + amount, plan_rank: 0})
                await getUserData()
            }catch(err){
                console.log(err)
            }
                ...
                <div className='fdrow aic gap02'>
                    ...
                    <div className='fdrow aic gap02'>
                        { userData[0]?.plan_profit === 0 ?
                            <p className='value'>${userData[0]?.plan_profit}.0</p>
                            :
                            <p className='value'>${userData[0]?.plan_profit.toFixed(1)}</p>
                        }
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7.27049 1.04714C7.66556 0.625727 8.33449 0.625725 8.72956 1.04714L15.0754 7.81608C15.6742 8.45475 15.2214 9.50002 14.3459 9.50002H11.5V10.5C11.5 11.0523 11.0523 11.5 10.5 11.5H5.50003C4.94774 11.5 4.50003 11.0523 4.50003 10.5V9.50002H1.65414C0.778704 9.50002 0.325857 8.45475 0.924607 7.81608L7.27049 1.04714Z" fill="green"/>
                            <path d="M4.50003 13.5C4.50003 12.9477 4.94774 12.5 5.50003 12.5H10.5C11.0523 12.5 11.5 12.9477 11.5 13.5V14.5C11.5 15.0523 11.0523 15.5 10.5 15.5H5.50003C4.94774 15.5 4.50003 15.0523 4.50003 14.5V13.5Z" fill="green"/>
                        </svg>
                    </div>
                </div>
            </div>
            ...
            <Routes>
                <Route index element={
                    ...
                    <h5 style={{marginBottom: '1rem'}} >Welcome, {getName()}</h5>
                    {
                        ...
                        <>
                            <StyledTable type={'admin_members'} data={getAllUsersData} isLoading={getAllUsersIsLoading} error={getAllUsersError} />
                        </>
                        :
                        <>
